...
ImageMagickCommands.prototype.resize = function (opts) {
	...
	var flag = opts.flag ? getResizeFlag(opts.flag) : '';
...
ImageMagickCommands.prototype.sharpen = function (opts) {
	...
	var amount = sharpening[mode][1];
	var threshold = sharpening[mode][2];
	var sigma = radius < 1 ? radius : Math.sqrt(radius);
...
ImageMagickCommands.prototype.exec = function (src, dst, callback) {
	...
	var promise = new Promise(function (resolve, reject) {
		exec(self.get(src, dst), function (err) {
			if (err) {
				return reject(err);
			}
