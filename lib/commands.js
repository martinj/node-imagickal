...
function pipeOrPath(obj) {
	return typeof (obj.pipe) === 'function' ? '-' : obj;
}
...
ImageMagickCommands.prototype.get = function (src, dst) {
	return [this.executable].concat([pipeOrPath(src)], this.commands, [pipeOrPath(dst)]).join(' ');
};
...
ImageMagickCommands.prototype.exec = function (src, dst, callback) {
	...
	const promise = new Promise((resolve, reject) => {
		const shell = exec(this.get(src, dst), { stdio: ['pipe', 'pipe'], maxBuffer: this.maxBuffer, encoding: 'binary' }, (err) => {
			if (err) {
				return reject(err);
			}
