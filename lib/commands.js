...
ImageMagickCommands.prototype.resize = function (opts) {
	...
	var height = opts.height || '',
		width = opts.width || '';
	this.commands.push(printf('-filter Catrom -resize %sx%s', width, height));
	return this;
};
...
ImageMagickCommands.prototype.rotate = function (opts) {
	...
	this.commands.push(printf(
		'-background %s -virtual-pixel background -distort ScaleRotateTranslate \'%s,%s %s\'',
 ...
 * @return {Promise}
 */
ImageMagickCommands.prototype.exec = function (src, dst) {
	var defer = Q.defer();
